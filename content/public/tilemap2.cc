...
void Tilemap2::BeforeTilemapComposite(bgfx::Encoder* encoder,
  ...
  if (atlas_need_update_) {
    CreateTileAtlasInternal(encoder, render_view);
...
void Tilemap2::CreateTileAtlasInternal(bgfx::Encoder* encoder,
                                       bgfx::ViewId* render_view) {
  if (bgfx::isValid(atlas_texture_.handle))
    bgfx::destroy(atlas_texture_.handle);

  atlas_texture_.size = base::Vec2i(tile_size_ * 64, tile_size_ * 32);
  ...
  if (screen()->api_version() >= APIVersion::RGSS3) {
    ...
    bgfx::updateTexture2D(
        bgfx::getTexture(atlas_texture_.handle), 0, 0,
        kShadowAtlasArea.x * tile_size_, kShadowAtlasArea.y * tile_size_,
        kShadowAtlasArea.width * tile_size_,
        kShadowAtlasArea.height * tile_size_,
        bgfx::copy(shadow_set->pixels, shadow_set->w * shadow_set->h * 4));
...
void Tilemap2::ParseMapDataBufferInternal() {
  ...
  auto process_quad = [&](base::RectF* texcoords, base::RectF* position,
    ...
    for (int i = 0; i < size; ++i)
      renderer::GeometryVertexLayout::SetTexPos(vert + i * 4, texcoords[i],
                                                position[i]);
  };
  ...
  if (!ground_vertices_.empty()) {
    memcpy(
        base_addr, ground_vertices_.data(),
        ground_vertices_.size() * sizeof(renderer::GeometryVertexLayout::Data));
  }

  if (!above_vertices_.empty()) {
    memcpy(
        base_addr + ground_vertices_.size(), above_vertices_.data(),
        above_vertices_.size() * sizeof(renderer::GeometryVertexLayout::Data));
  }

  tilemap_quads_->Update();
  screen()->device()->quad_indices()->EnsureSize(quad_size);
}

void Tilemap2::DrawFlashLayerInternal(CompositeTargetInfo* target_info) {
...
void Tilemap2::DrawGroundLayerInternal(CompositeTargetInfo* target_info) {
  int ground_quad_size = ground_vertices_.size() / 4;
  if (!ground_quad_size)
    return;

  auto& shader = screen()->device()->pipelines().tilemap2;
  ...
  base::Vec4 utilesize;
  utilesize.x = tile_size_;
  target_info->encoder->setUniform(shader.TileSize(), &utilesize);

  base::Vec4 anim_offset;
  anim_offset.x = animation_offset_.x;
  anim_offset.y = animation_offset_.y;
  target_info->encoder->setUniform(shader.AnimOffset(), &anim_offset);
...
void Tilemap2::DrawZLayerInternal(CompositeTargetInfo* target_info) {
  int ground_quad_size = ground_vertices_.size() / 4;
  int above_quad_size = above_vertices_.size() / 4;
  ...
  tilemap_quads_->Draw(target_info->encoder, shader.GetProgram(),
...
void Tilemap2::InitDrawableData() {
  ...
  atlas_texture_ = BGFX_INVALID_HANDLE;
}
