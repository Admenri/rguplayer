...
int Sound_Init(void) {
  ...
  available_decoders =
      (const Sound_DecoderInfo**)SDL_calloc(total, sizeof(Sound_DecoderInfo*));
 ...
 */
static Sound_Sample* alloc_sample(SDL_IOStream* rw,
...
static int init_sample(const Sound_DecoderFunctions* funcs,
  ...
  if (_desired == NULL)
    ...
    desired.format =
        _desired->format ? _desired->format : sample->actual.format;
    desired.channels =
        _desired->channels ? _desired->channels : sample->actual.channels;
    ...
    if ((sample->actual.format != desired.format) ||
      ...
      if (internal->stream == NULL) {
        ...
        funcs->close(sample);
        SDL_SeekIO(internal->rw, pos,
                   SDL_IO_SEEK_SET); /* set for next try... */
        return 0;
      } /* if */
    }   /* if */
  }     /* else */
  ...
  SNDDBG(("New sample DESIRED format: %s format, %d rate, %d channels.\n",
          fmt_to_str(sample->desired.format), sample->desired.rate,
          sample->desired.channels));

  SNDDBG(("New sample ACTUAL format: %s format, %d rate, %d channels.\n",
          fmt_to_str(sample->actual.format), sample->actual.rate,
...
Sound_Sample* Sound_NewSample(SDL_IOStream* rw,
                              const char* ext,
                              Sound_AudioInfo* desired,
                              Uint32 bSize) {
  Sound_Sample* retval;
...
Uint32 Sound_Decode(Sound_Sample* sample) {
  ...
  while ((available = SDL_GetAudioStreamAvailable(internal->stream)) <
    ...
    if ((br > 0) && (SDL_PutAudioStreamData(internal->stream, internal->buffer,
                                            (int)br) == -1)) {
      __Sound_SetError(SDL_GetError());
      sample->flags |= SOUND_SAMPLEFLAG_ERROR;
      return 0; /* oh well. */
  ...
  if (available > 0) {
    ...
    const int br =
        SDL_GetAudioStreamData(internal->stream, sample->buffer, readlen);
    if (br != readlen) {
      __Sound_SetError(SDL_GetError());
      sample->flags |= SOUND_SAMPLEFLAG_ERROR;
      return 0; /* oh well. */
...
/* This falls back to an included copy/paste of SDL's SIMDAlloc code if you
   aren't using a new enough SDL. To keep this simple, the included copy assumes
   you need to align to 64 bytes, which is a little wasteful but should work on
   everything from MMX to AVX-512. The real SDL checks the CPU at runtime to
   decide what's available and aligns to smaller numbers if all you have is SSE,
   Altivec or NEON. Not to mention this is just a second copy of the code that
   doesn't get attention...you should really
