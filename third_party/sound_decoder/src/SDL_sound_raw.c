...
static int RAW_open(Sound_Sample* sample, const char* ext) {
   ...
   */
  if ((sample->desired.channels < 1) || (sample->desired.channels > 2) ||
      (sample->desired.rate == 0) || (sample->desired.format == 0)) {
...
static Uint32 RAW_read(Sound_Sample* sample) {
   ...
   */
  retval = SDL_ReadIO(internal->rw, internal->buffer, internal->buffer_size);
...
static int RAW_rewind(Sound_Sample* sample) {
  Sound_SampleInternal* internal = (Sound_SampleInternal*)sample->opaque;
  BAIL_IF_MACRO(SDL_SeekIO(internal->rw, 0, SDL_IO_SEEK_SET) != 0, ERR_IO_ERROR,
                0);
  return 1;
} /* RAW_rewind */
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_RAW = {
    {extensions_raw, "Raw audio", "Ryan C. Gordon <icculus@icculus.org>",
