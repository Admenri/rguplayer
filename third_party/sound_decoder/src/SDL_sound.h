...
/**
 * \fn Sound_Sample *Sound_NewSample(SDL_IOStream *rw, const char *ext,
 * Sound_AudioInfo *desired, Uint32 bufferSize) \brief Start decoding a new
 * sound sample.
 *
...
/**
 * \fn Sound_Sample *Sound_NewSampleFromMem(const Uint8 *data, Uint32 size,
 * const char *ext, Sound_AudioInfo *desired, Uint32 bufferSize) \brief Start
 * decoding a new sound sample from a file on disk.
 *
...
Sound_NewSampleFromMem(const Uint8* data,
                       Uint32 size,
                       const char* ext,
                       Sound_AudioInfo* desired,
                       Uint32 bufferSize);
 ...
 * Sound_AudioInfo *desired, Uint32 bufferSize) \brief Start decoding a new
 * sound sample from a file on disk.
 *
 * This is identical to Sound_NewSample(), but it creates an SDL_IOStream for
 * you from the file located in (filename). Note that (filename) is specified in
 ...
 */
SNDDECLSPEC Sound_Sample* SDLCALL
Sound_NewSampleFromFile(const char* fname,
                        Sound_AudioInfo* desired,
                        Uint32 bufferSize);
 ...
 *  that a streaming source (for example, from a SDL_IOStream that is getting
