...
static size_t mp3_read(void* pUserData, void* pBufferOut, size_t bytesToRead) {
  ...
  /* !!! FIXME: dr_mp3 treats returning less than bytesToRead as EOF. So we
   * can't EAGAIN. */
  while (bytesToRead) {
...
static drmp3_bool32 mp3_seek(void* pUserData,
  ...
  const int whence =
      (origin == drmp3_seek_origin_start) ? SDL_IO_SEEK_SET : SDL_IO_SEEK_CUR;
  Sound_Sample* sample = (Sound_Sample*)pUserData;
  Sound_SampleInternal* internal = (Sound_SampleInternal*)sample->opaque;
  return (SDL_SeekIO(internal->rw, offset, whence) != -1) ? DRMP3_TRUE
                                                          : DRMP3_FALSE;
} /* mp3_seek */
...
static Uint32 MP3_read(Sound_Sample* sample) {
  ...
  drmp3* dr = (drmp3*)internal->decoder_private;
  const drmp3_uint64 frames_to_read =
      (internal->buffer_size / channels) / sizeof(float);
  const drmp3_uint64 rc =
      drmp3_read_pcm_frames_f32(dr, frames_to_read, (float*)internal->buffer);
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_MP3 = {
    {extensions_mp3, "MPEG-1 Audio Layer I-III",
