...
static size_t flac_read(void* pUserData, void* pBufferOut, size_t bytesToRead) {
  /* !!! FIXME: dr_flac treats returning less than bytesToRead as EOF. So we
   * can't EAGAIN. */
...
static drflac_bool32 flac_seek(void* pUserData,
                               int offset,
                               drflac_seek_origin origin) {
  const int whence =
      (origin == drflac_seek_origin_start) ? SDL_IO_SEEK_SET : SDL_IO_SEEK_CUR;
  Sound_Sample* sample = (Sound_Sample*)pUserData;
  Sound_SampleInternal* internal = (Sound_SampleInternal*)sample->opaque;
  return (SDL_SeekIO(internal->rw, offset, whence) != -1) ? DRFLAC_TRUE
                                                          : DRFLAC_FALSE;
} /* flac_seek */
...
static int FLAC_open(Sound_Sample* sample, const char* ext) {
  ...
  if (dr->totalPCMFrameCount == 0)
    ...
    internal->total_time +=
        ((dr->totalPCMFrameCount % dr->sampleRate) * 1000) / dr->sampleRate;
  } /* else */
...
static Uint32 FLAC_read(Sound_Sample* sample) {
  ...
  drflac* dr = (drflac*)internal->decoder_private;
  const drflac_uint64 frames_to_read =
      (internal->buffer_size / channels) / sizeof(drflac_int32);
  const drflac_uint64 rc = drflac_read_pcm_frames_s32(
      dr, frames_to_read, (drflac_int32*)internal->buffer);
  /* !!! FIXME: we only set the EOF flags, but this only tells you we're done,
   * not about i/o errors, nor corruption. */
...
static int FLAC_seek(Sound_Sample* sample, Uint32 ms) {
  ...
  const drflac_uint64 frame_offset =
      (drflac_uint64)(frames_per_ms * ((float)ms));
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_FLAC = {
    {extensions_flac, "Free Lossless Audio Codec",
