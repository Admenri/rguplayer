...
#define FMT_IEEE_FLOAT \
  0x0003 /* Uncompressed IEEE floating point waveform data. */
...
static Uint32 read_sample_fmt_normal(Sound_Sample* sample) {
  ...
  if (w->fmt->wBitsPerSample == 24) {
    ...
    /* we're going to expand by 25%...3 bytes to 4. Make sure the buffer has
     * room to expand. */
  ...
  if ((retval > 0) && (w->fmt->wBitsPerSample == 24)) {
    ...
    for (i = 0; i < total; i++, dst--, src -= 3) {
      const Uint32 sample =
          ((Uint32)src[0]) | (((Uint32)src[1]) << 8) | (((Uint32)src[2]) << 16);
      *dst = sample << 8; /* shift it up so the most significant bits cover the
                             32-bit space. */
    }
    retval = total * 4;
  }
...
static SDL_INLINE void do_adpcm_nibble(Uint8 nib,
  ...
  static const Sint32 AdaptionTable[] = {230, 230, 230, 230, 307, 409,
  ...
  delta =
      ((Sint32)header->iDelta * AdaptionTable[nib]) / FIXED_POINT_ADAPTION_BASE;
...
static SDL_INLINE int decode_adpcm_sample_frame(Sound_Sample* sample) {
  ...
  for (i = 0; i < max; i++) {
    ...
    Sint32 lPredSamp =
        ((headers[i].iSamp1 * iCoef1) + (headers[i].iSamp2 * iCoef2)) /
...
static int WAV_open_internal(Sound_Sample* sample,
                             const char* ext,
                             fmt_t* fmt) {
  Sound_SampleInternal* internal = (Sound_SampleInternal*)sample->opaque;
  SDL_IOStream* rw = internal->rw;
  ...
  uint32_t v;
  SDL_ReadU32LE(rw, &v);
  BAIL_IF_MACRO(v != riffID, "WAV: Not a RIFF file.", 0);
  SDL_ReadU32LE(rw, &v); /* throw the length away; we get this info later. */
  SDL_ReadU32LE(rw, &v);
  BAIL_IF_MACRO(v != waveID, "WAV: Not a WAVE file.", 0);
  ...
  if (fmt->wFormatTag == FMT_IEEE_FLOAT) {
    BAIL_IF_MACRO(fmt->wBitsPerSample != 32, "WAV: Unsupported sample size.",
                  0);
  ...
  fmt->sample_frame_size =
      (((sample->actual.format & 0xFF) / 8) * sample->actual.channels);
  internal->decoder_private = (void*)w;

  if (fmt->dwAvgBytesPerSec ==
      0) { /* we assume data is uncompressed if this field is unset. */
  ...
  internal->total_time +=
      (fmt->total_bytes % fmt->dwAvgBytesPerSec) * 1000 / fmt->dwAvgBytesPerSec;
...
static int WAV_rewind(Sound_Sample* sample) {
  ...
  fmt_t* fmt = w->fmt;
  const Sint64 rc =
      SDL_SeekIO(internal->rw, fmt->data_starting_offset, SDL_IO_SEEK_SET);
...
const Sound_DecoderFunctions __Sound_DecoderFunctions_WAV = {
    {extensions_wav, "Microsoft WAVE audio format",
     "Ryan C. Gordon <icculus@icculus.org>", "https://icculus.org/SDL_sound/"},
