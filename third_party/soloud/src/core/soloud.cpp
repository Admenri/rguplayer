...
namespace SoLoud
{
	...
	{		
		...
		if (aSamplerate != 0) samplerate = aSamplerate;
		if (aBufferSize != 0) buffersize = aBufferSize;

		if (callback)
		{
			if (aBufferSize == 0) buffersize = 2048;

			int ret = callback(this, aFlags, samplerate, buffersize, aChannels);
			if (ret == 0)
				inited = 1;
