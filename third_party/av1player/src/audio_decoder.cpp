...
AudioDecoder::AudioDecoder(VideoPlayer* parent, size_t bufferSize)
    : m_packetCount(0), m_parent(parent), m_framesDecoded(0) {
...
bool AudioDecoder::initHeader(unsigned char* data, size_t size) {
  ...
  debugLog("vorbis header packet 0 type: %d (%c %c %c %c %c %c)", type, data[1],
  ...
  debugLog("vorbis header packet 1 type: %d (%c %c %c %c %c %c)", type, data[1],
           data[2], data[3], data[4], data[5], data[6]);
  ...
  debugLog("vorbis header packet 2 type: %d (%c %c %c %c %c %c)", type, data[1],
           data[2], data[3], data[4], data[5], data[6]);
...
int AudioDecoder::decode(const unsigned char* aData,
                         size_t aLength,
                         int64_t aOffset,
                         uint64_t aTstampUsecs,
                         uint64_t aTstampDuration,
                         int64_t aDiscardPadding,
                         int32_t* aTotalFrames) {
  if (m_packetCount < 3)
    return 1;

  ogg_packet pkt =
      InitOggPacket(aData, aLength, false, false, -1, m_packetCount++);
